...
Since yesterday, I added a feature to only output alternative locations for integrations if the edit distance for that alternative integration is close to the edit distance for the primary integraton.

This should hopefully reduce the number of false negatives, since there should be fewer integrations that get sent to the 'host_ambig' file.
...
If we use a different kind of distance: 'd_shortest' (bedtools closest) then isling is the best:
...
So the results aren't amazing: the jaccard is low, the TPR is a bit better than vseq-tookit but the PPV is lower.

What's going on with the jaccard statistic?
Maybe it's because there are some long output intervals??  Check the length of the intervals in one of the output files:

```{r}
test <- read_tsv("../out/experiment1_OTC_chr1/AAV-OTC/AAV_analysis2/ints_for_scoring/cond0.rep0.hg38.AAV2.integrations.post.merged.bed", col_names = c("chr", "start", "stop"))
...
There seem to be some long intervals (some > 1kb).  Why is that?
...
This could be because we're using a long template length when running the script.  Check the template length found for the host and virus alignments

```{bash}
echo 'host alignment:'
cat ../out/experiment1_OTC_chr1/AAV-OTC/AAV_analysis2/host_stats/cond0.rep0.hg38.readsFromAAV2.txt | grep 'insert size average'

echo 'virus alignment:'
cat ../out/experiment1_OTC_chr1/AAV-OTC/AAV_analysis2/virus_stats/cond0.rep0.AAV2.txt | grep 'insert size average'
```

So this is why we have some long intervals: these are probably discordant pairs, and we're using a template length that's the average of the host and virus alignment average template lengths. Instead, it would be better to use the actual template length from the simiulation




AAV_analysis2/ints_for_scoring/cond0.rep0.hg38.AAV2.integrations.post.merged.bed

Why are there more false positives? - probably also the distances thing

First, check out the scores for d_coords_min for isling:
...
int_scores %>% 
  ...
  filter(window == 5) %>% 
  filter(tool == "isling") %>% 
  filter(score_type == "d_coords_min")
```

The false positives come from the found-to-sim distances.  
