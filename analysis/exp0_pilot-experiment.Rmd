...
```{r include=FALSE}
results <- importData(sim[1],  analysis[1], scored[1])
for (i in seq(2, length(sim))){
  tmp <- importData(sim[i],  analysis[i], scored[i])
...
First check out the false negatives - which integrations do they come from?
```{bash, engine.opts='-e', class.output="scroll-100"}
echo " count int_id  type"
awk 'match($3, /fn/)' "../out/experiment0_prelim/rep68-easier/scored_reads/rep68-easier_analysis0.cond0.rep0.chr1.rep68.post.tsv" | cut -f2,10 | sort -k1,1n -k2 | uniq -c
```


If we plot a histogram the number of reads missing from each integration, it looks like this:

```{bash, engine.opts='-e', include=FALSE}
awk 'match($3, /fn/)' "../out/experiment0_prelim/rep68-easier/scored_reads/rep68-easier_analysis0.cond0.rep0.chr1.rep68.post.tsv" | cut -f2,10 | sort -k1,1n -k2 | uniq -c > tmp.tsv
```


```{r include=FALSE}
fn_ints <- read_table2("tmp.tsv", col_names = c("n_missing", "int_id", "type"))
```


```{r}
fn_ints %>% 
  ggplot(aes(x = n_missing, color = type)) +
  geom_freqpoly()
```


So it's definetly the case that most integrations that have missing reads have only one or two reads missing, but a few have a lot of missing reads.  It would be good to examine the properties of these integrations, to see if there's anything consistent about the integrations with a lot of missing reads.  Since these are clean, whole integrations with no deletions or rearrangements, the only thing to examine is the position in the host.  So it would be good to plot the density of missing integrations within the host chromosome.


```{r include=FALSE}
ints <- read_tsv("../out/experiment0_prelim/rep68-easier/sim_ints/cond0.rep0.int-info.annotated.tsv")

fn_ints <- fn_ints %>% 
  pivot_wider(names_from = 'type', values_from = 'n_missing')

ints <- left_join(ints, fn_ints, by = c("id" = "int_id"))

ints <- ints %>% 
  mutate(chimeric = replace_na(chimeric, 0)) %>% 
  mutate(discord = replace_na(discord, 0))
```

As a first step, just categorise integrations based on if they having missing reads or not.  Plot the density of both types of integrations across chromosome 1.

```{r}
ints %>% 
  mutate(missing_reads = (chimeric != 0 | discord != 0)) %>% 
  ggplot(aes(x=hPos, color=missing_reads)) +
  geom_density(bw=1e4)
```


But it would be nice to be able to weight these densities in terms of how many integrations are missing. Make bins of width 1000 that span the length of the chromosome and count how many integrations are in each
...
tmp <- ints %>% 
  group_by(bin = cut(hPos, breaks=seq(0, width*n_bins, width), labels= seq(0, n_bins-1))) 

tmp %>% 
  summarise(chimeric = sum(chimeric),
            discordant = sum(discord)) %>% 
  mutate(start = as.numeric(as.character(bin))*width) %>% 
  pivot_longer(chimeric:discordant, names_to = "type", values_to = "count") %>% 
  ggplot(aes(x = start, y = count, color = type)) +
  geom_line() + 
  facet_grid(vars(type))

```
There's a few things here. It looks like a lot of the misisng reads are just sprinkled thoroughout the chromosome.  There's a big patch in the middle of the chromosome (centromere??) with no missing reads.
...
tmp %>% 
  ungroup() %>% 
  rowwise() %>% 
  mutate(chimeric_reads = paste0(left_chimeric, ";", right_chimeric)) %>% 
  mutate(discord_reads = paste0(left_discord, ";", right_discord)) %>% 
  select(-one_of("left_discord", "right_discord", "left_chimeric", "right_chimeric")) %>% 
  ungroup() %>% 
  pivot_longer(chimeric_reads:discord_reads, names_to = "read_type", values_to = "read_names") %>% 
  mutate(read_names = str_replace(read_names, ";NA", "")) %>% 
  mutate(read_names = str_replace(read_names, "NA;", "")) %>% 
  rowwise() %>% 
  mutate(read_count = length(str_split(read_names, ";")[[1]])) %>% 
  mutate(read_count = ifelse(str_detect(read_names, "NA"), 0, read_count)) %>% 
  ungroup() %>% 
  pivot_wider(names_from = "read_type", values_from = c("read_names", "read_count")) %>% 
  group_by(bin) %>% 
  summarise(frac_missing_chimeric = chimeric/read_count_chimeric_reads,
            frac_missing_discord = discord/read_count_discord_reads) 
...
Start with the discordant pairs, and check out the top 10 bins with missing discordant pairs:
```{r}
tmp %>% 
  summarise(n_chimeric_missing = sum(chimeric),
            n_discordant_missing = sum(discord)) %>% 
  arrange(desc(n_discordant_missing)) %>% 
  slice(1:10) %>% 
  mutate(bin = as.numeric(as.character(bin))) %>% 
  mutate(start = bin*width) %>% 
  mutate(stop = (bin+1)*width)
```

Take the top bin - which integrations are in this bin

```{r}

ints %>% 
  filter(hPos >= 120020000 & hPos <= 120030000) %>% 
  mutate(chimeric_reads = paste0(left_chimeric, ';', right_chimeric)) %>% 
  mutate(discordant_reads =  paste0(left_discord, ';', right_discord)) %>% 
  mutate(n_chimeric_reads = length(str_split(chimeric_reads, ";")[[1]])) %>% 
    mutate(n_chimeric_reads = length(str_split(discordant_reads, ";")[[1]]))  
