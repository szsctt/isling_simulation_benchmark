...
Now looking at the three replicates of rep68 inserted into chr1 (and analysed against chr1) together.  If we plot a histogram the number of reads missing from each integration, it looks like this:



```{r include=FALSE}
scored_files <- Sys.glob("../out/experiment0_prelim/rep68-easier/scored_reads/rep68-easier_analysis0.cond0.rep*.chr1.rep68.tsv")
int_files <- Sys.glob("../out/experiment0_prelim/rep68-easier/sim_ints/cond0.rep*.int-info.annotated.tsv")

i <- 1
cond <- str_extract(basename(int_files[i]), "cond\\d\\.rep\\d")
fn_ints <-  annotate_ints_matching_condition("3", "fn", scored_files[i], int_files[i]) %>% 
  mutate(condition = cond)
  
for (i in seq(2, length(scored_files))) {
  cond <- str_extract(basename(int_files[i]), "cond\\d\\.rep\\d")
  tmp <- annotate_ints_matching_condition("3", "fn", scored_files[i], int_files[i]) %>% 
    mutate(condition = cond)
  fn_ints <- bind_rows(fn_ints, tmp)
}
...
fn_ints %>% 
  ...
  filter(n_missing >= 1) %>% 
  ggplot(aes(x = n_missing, fill = condition)) +
  geom_bar() +
...
#
fn_ints <- add_bins_to_annotated_ints(fn_ints, width, chr_len)
...
fn_ints %>% 
  ...
  ggplot(aes(x = pos, y = frac, color = type)) +
  geom_line() +
...
for l in $LIST; do
  echo "viral alignment:"
  python3 python_scripts/print_cigars_from_sam.py ../out/experiment0_prelim/rep68-easier_analysis0_chr1_rep68/virus_aligned/cond0.rep2.rep68.sam  $l
  echo "host_alignment:"
  python3 python_scripts/print_cigars_from_sam.py ../out/experiment0_prelim/rep68-easier_analysis0_chr1_rep68/host_aligned/cond0.rep2.chr1.readsFromrep68.sam $l  
  echo 
