...
Again, these false positives seem to come from integrations 3 and 4.  Check out the reads that are only f
...
tmp <- ints %>% 
  group_by(bin = cut(hPos, breaks=seq(0, width*n_bins, width), labels(seq(n_bins)))) 

tmp %>% 
  summarise(chimeric = sum(chimeric),
            discordant = sum(discord)) %>% 
  mutate(bin = as.numeric(bin)) %>% 
  mutate(start = width*bin) %>% 
