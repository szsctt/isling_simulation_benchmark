...
int_scores <- importIntScoreExperiment(exp_dir) %>% 
  distinct() %>% 
  filter(!str_detect(experiment, "OTC-vect-easier")) 
...
int_scores <- int_scores %>% 
  ...
  mutate(experient = case_when(
    experiment == "OTC_vect-harder" ~ "OTC-harder",
    TRUE ~ experiment
  ))
...
```{r}
int_scores %>% 
  select(experiment, replicate, analysis_host, post, analysis_tool, `merged-ints`, TPR, PPV, tp:fn) %>% 
...
p <- int_scores %>% 
  ...
  xlim(0, 1) +
  ylim(0, 1) +
  facet_grid(vars(`merged-ints`),vars(experiment))
print(p)
```
