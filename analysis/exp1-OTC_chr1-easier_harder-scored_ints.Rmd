...
exp_dir <- "../out/experiment1_OTC_chr1/easier_harder"
```
...
![DAG for this experiment](../out/experiment1_OTC_chr1/easier-harder.dag.svg)
...
```{r include=FALSE}
scored_int_files <- list.files(exp_dir, pattern = "scored_ints_summary.tsv", recursive = TRUE)

int_scores <- tibble(
  file = list.files(exp_dir, pattern = "scored_ints_summary.tsv", recursive = TRUE)
) %>% 
...
analysis_conditions <- read_tsv("../out/experiment1_OTC_chr1/easier_harder/OTC-vect-easier/analysis_conditions.tsv")
...
```{r}
int_scores %>% 
  select(experiment, replicate, analysis_host, post, analysis_tool, merge, TPR, PPV, tp:fn) %>% 
  arrange(analysis_tool, replicate, analysis_host)
```
...
```{r include=FALSE}
p <- int_scores %>% 
  ggplot(aes(x = PPV, y = TPR, color = analysis_tool, shape=post)) +
  geom_point() +
  xlim(0, 1) +
  ylim(0, 1) +
  facet_grid(vars(analysis_host), vars(merge)) +
    transition_states(experiment,
                    transition_length = 2,
                    state_length = 1) +
  enter_fade() +
  exit_shrink() +
  ...
  labs(title = "Chimeric reads",
      subtitle = "Simulation condition: {closest_state}") 

anim_save("animations/expt1_easier-harder_int-scores.gif", p)
```
![](animations/expt1_easier-harder_int-scores.gif)

So the three different pipelines seem to have quite different performances.  

Of the three tools, Polyidus has the best positive predictive value for both conditions.  However, this comes at the cost of missing most of the integrations - the true positive rate for this tool is around 25%.  

The performance of ViFi varies a lot between the two conditions.  In the easier condition, the true positive rate isn't too bad (around 75%), but the positive predictive value around 50%, so it doesn't miss too many integations, but there are quite a few false positives.  In the harder condition, the true positive rate goes way down (so a lot of integrations are missed), but the positive predictive value doesn't change too much.
...
First, check all the false positives that were identified, and see if the reads in those false positives are actually indicated to cross integrations.

```{r include=FALSE}
exps <- c("OTC-vect-harder", "OTC-vect-easier")
folder <- "../out/experiment1_OTC_chr1/easier_harder"

filenames <- c()

for (exp in exps) {
  scored_ints_folder <- file.path(folder, exp, "scored_ints")
...
A reminder of how those analysis conditions are different:
```{r}
analysis_conditions %>% 
  select(experiment:host, virus, merge) %>% 
  filter(str_detect(analysis_condition, "analysis"))
```
...
for (exp in exps) {
  this_folder <- file.path(folder, exp)
...
fp_dists <- int_scores %>% 
  ...
  mutate(data = map(fp_closest_file, ~read_tsv(file.path(folder, exp, "scored_ints", .),
