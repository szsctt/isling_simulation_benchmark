---
title: "Results for publication"
output: 
  html_notebook:
    code_folding: hide
---


```{r setup, include=FALSE}
library(tidyverse)
library(ggpubr)
library(kableExtra)

source("sim_functions.R")

results_dir <- "../out/experiment1_OTC_chr1"
```

```{css, include=FALSE}
.scroll_100 {
  max-height: 300px;
  overflow-y: auto;
  background-color: inherit;
}
```


# Overview
I conducted several experiments in order to investigate the performatce of the pipeline, and compare it to similar tools.

## 'Easier' and 'harder'

The first experiment simulated some 'easier' and some 'harder' conditions.  I also tried out some wild-type virus (AAV2) and vector (OTC vector).

### Data
The only data required for simulation experiments are host and virus references.  

For the host, I used chromosome 1 from hg38.   

For the virus I used the OTC vector genome (the subsequence of the plasmid from ITR to ITR)

### Simulation parameters - easier and harder

I simulated two different situations using this data: one that I thought would be 'easier' for the pipeline to handle, and one that is 'harder'.

These conditions are summarised in the config file:

```{bash attr.output='style="max-height: 300px;"'}
cat ../config/experiment1_OTC_chr1/analysis-conditions.yml
```


### Comparison with other tools

We can compare the performance of our pipeline with other tools

For each replicate, we can calulate the positive predictive value (how many of the predicted positves are true positves - high means few false positves), and the true positve rate (how many of the real positves are detected - high means few false negatives).


We can calculate the true positive rate (sensitivity, recall) as
$$TPR = \frac{TP}{P} = \frac{TP}{TP+FN}$$

And the positive predictive value as
$$PPV = \frac{TP}{TP+FP}$$

```{r include=FALSE}
exp_dir <- file.path(results_dir, "easier-harder")
int_scores <- importIntScoreExperiment(exp_dir) 

# only use postprocessed data from our pipeline
int_scores <- int_scores %>% 
  filter(case_when((analysis_tool != "pipeline") ~ TRUE,
                    post ~ TRUE,
                    TRUE ~ FALSE
    )) 
```

First, a table of results:
```{r}
int_scores %>% 
  select(experiment, replicate, window, coords_score_type, analysis_tool, TPR, PPV, tp:fn, results_file) %>% 
  arrange(experiment, analysis_tool, replicate, window) 
```

And also, a plot:
```{r}
p <- int_scores %>% 
  ggplot(aes(x = PPV, y = TPR, color = analysis_tool, shape = coords_score_type)) +
  geom_point() +
  xlim(0, 1) +
  ylim(0, 1) +
  facet_grid(vars(experiment), vars(window))
print(p)
```

```{r}
p <- int_scores %>% 
  filter(coords_score_type == "coords") %>% 
  filter(window == 20) %>% 
  ggplot(aes(x = PPV, y = TPR, color = analysis_tool, shape = coords_score_type)) +
  geom_point() +
  xlim(0, 1) +
  ylim(0, 1) +
  facet_wrap(vars(experiment))
print(p)
```



We can instead use ggpubr to make the plot:


```{r}
p <- ggscatter(int_scores, x = "PPV", y = "TPR", color = "analysis_tool",
               facet.by = c("experiment", "window"))
ggpar(p, xlim= c(0, 1), ylim = c(0, 1), xlab = "Positive predictive value", ylab = "True positive rate",
      xtickslab.rt = 45)
```

Really, we only care about the 'harder' conditions, and we won't merge nearby sites:

```{r}
p <- ggscatter(int_scores, x = "PPV", y = "TPR", color = "analysis_tool",
               facet.by = c("experiment", "window"))
ggpar(p, xlim= c(0, 1), ylim = c(0, 1), xlab = "Positive predictive value", ylab = "True positive rate")
```


### Simulation parameters - chromosomes

I also explored the effect of using different chromosomes as references for integraion.  I always used hg38 for analysis: see config file below:

```{bash attr.output='style="max-height: 300px;"'}
cat ../config/experiment1_OTC_chr1/chromosomes.yml
```

```{r include=FALSE}
exp_dir <- file.path(results_dir, "chromosomes")
int_scores <- importIntScoreExperiment(exp_dir) 


# only use postprocessed data from our pipeline
int_scores <- int_scores %>% 
  filter(case_when((analysis_tool != "pipeline") ~ TRUE,
                    post ~ TRUE,
                    TRUE ~ FALSE
    )) %>% 
  filter(window == 20) %>%   
  filter(coords_score_type == "coords") 
```


```{r}
int_scores %>% 
  select(experiment, replicate, host_name, post, analysis_tool, window, TPR, PPV, tp:fn) %>% 
  arrange(experiment, analysis_tool, replicate, host_name, window) 
```

```{r}
for (chr in unique(int_scores$host_name)) {
p <- int_scores %>% 
  filter(host_name == chr) %>% 
  ggplot(aes(x = PPV, y = TPR, color = analysis_tool)) +
  geom_point() +
  xlim(0, 1) +
  ylim(0, 1) +
  facet_grid(vars(experiment), vars(window)) + 
  ggtitle(chr)
print(p)
}
```

### Simulation parameters - integration properties

I also explored the effect kinds of integrations: see config file below:

```{bash attr.output='style="max-height: 300px;"'}
cat ../config/experiment1_OTC_chr1/condition-breakdown.yml
```



```{r include=FALSE}
exp_dir <- file.path(results_dir, "condition-breakdown")
int_scores <- importIntScoreExperiment(exp_dir) 


# only use postprocessed data from our pipeline
int_scores <- int_scores %>% 
  filter(case_when((analysis_tool != "pipeline") ~ TRUE,
                    post ~ TRUE,
                    TRUE ~ FALSE
    ))  %>% 
  filter(window == 20) %>%   
  filter(coords_score_type == "coords") 
```


```{r}
int_scores %>% 
  select(experiment, condition, replicate, post, analysis_tool, window, TPR, PPV, tp:fn) %>% 
  arrange(experiment, analysis_tool, condition, replicate,  window) 
```

The experiments completed so far are:
```{r}
unique(int_scores$experiment)
```

### Proportion of integrations with deletions
```{r}
int_scores %>% 
  filter(experiment == "OTC-deletion") %>% 
  group_by(condition, experiment, tool, window, p_delete) %>% 
  summarise(TPR = mean(TPR),
            TPR_sd = sd(TPR),
            PPV = mean(PPV),
            PPV_sd = sd(PPV),
            n= n()) %>% 
  ggplot(aes(x = p_delete, y = PPV, color = tool)) +
  geom_point(alpha = 0.5) +
  geom_line() +
  xlim(0, 1) +
  ylim(0, 1) +
  facet_wrap(vars(window))
```


```{r}
int_scores %>% 
  filter(experiment == "OTC-deletion") %>% 
  group_by(condition, experiment, tool, window, p_delete) %>% 
  summarise(TPR = mean(TPR),
            TPR_sd = sd(TPR),
            PPV = mean(PPV),
            PPV_sd = sd(PPV),
            n= n()) %>% 
  ggplot(aes(x = p_delete, y = TPR, color = tool)) +
  geom_point(alpha = 0.5) +
  geom_line() +
  xlim(0, 1) +
  ylim(0, 1) +
  facet_wrap(vars(window))
```

```{r}
int_scores %>% 
  filter(experiment == "OTC-fcov") %>% 
  ggplot(aes(x = PPV, y = TPR, color = tool, shape=as.factor(fcov))) +
  geom_point(alpha = 0.5) +
  facet_wrap(vars(window))
```


### Proportion of integrations with rearrangements
```{r}
int_scores %>% 
  filter(experiment == "OTC-rearrange") %>% 
  group_by(condition, experiment, tool, window, p_rearrange) %>% 
  summarise(TPR = mean(TPR),
            TPR_sd = sd(TPR),
            PPV = mean(PPV),
            PPV_sd = sd(PPV),
            n= n()) %>% 
  ggplot(aes(x = p_rearrange, y = PPV, color = tool)) +
  geom_point(alpha = 0.5) +
  geom_line() +
  xlim(0, 1) +
  ylim(0, 1) +
  facet_wrap(vars(window))
```

```{r}
int_scores %>% 
  filter(experiment == "OTC-rearrange") %>% 
  group_by(condition, experiment, tool, window, p_rearrange) %>% 
  summarise(TPR = mean(TPR),
            TPR_sd = sd(TPR),
            PPV = mean(PPV),
            PPV_sd = sd(PPV),
            n= n()) %>% 
  ggplot(aes(x = p_rearrange, y = TPR, color = tool)) +
  geom_point(alpha = 0.5) +
  geom_line() +
  xlim(0, 1) +
  ylim(0, 1) +
  facet_wrap(vars(window))
```

### Proportion of integrations that are whole
```{r}
int_scores %>% 
  filter(experiment == "OTC-whole") %>% 
  group_by(condition, experiment, tool, window, p_whole) %>% 
  summarise(TPR = mean(TPR),
            TPR_sd = sd(TPR),
            PPV = mean(PPV),
            PPV_sd = sd(PPV),
            n= n()) %>% 
  ggplot(aes(x = p_whole, y = PPV, color = tool)) +
  geom_point(alpha = 0.5) +
  geom_line() +
  xlim(0, 1) +
  ylim(0, 1) +
  facet_wrap(vars(window))
```
```{r}
int_scores %>% 
  filter(experiment == "OTC-whole") %>% 
  group_by(condition, experiment, tool, window, p_whole) %>% 
  summarise(TPR = mean(TPR),
            TPR_sd = sd(TPR),
            PPV = mean(PPV),
            PPV_sd = sd(PPV),
            n= n()) %>% 
  ggplot(aes(x = p_whole, y = TPR, color = tool)) +
  geom_point(alpha = 0.5) +
  geom_line() +
  xlim(0, 1) +
  ylim(0, 1) +
  facet_wrap(vars(window))
```

### Minimum length of integrations
```{r}
int_scores %>% 
  filter(experiment == "OTC-min_len") %>% 
  group_by(condition, experiment, tool, window, min_len) %>% 
  summarise(TPR = mean(TPR),
            TPR_sd = sd(TPR),
            PPV = mean(PPV),
            PPV_sd = sd(PPV),
            n= n()) %>% 
  ggplot(aes(x = min_len, y = PPV, color = tool)) +
  geom_point(alpha = 0.5) +
  geom_line() +
  facet_wrap(vars(window))  + 
  ylim(0, 1) 
```

```{r}
int_scores %>% 
  filter(experiment == "OTC-min_len") %>% 
  group_by(condition, experiment, tool, window, min_len) %>% 
  summarise(TPR = mean(TPR),
            TPR_sd = sd(TPR),
            PPV = mean(PPV),
            PPV_sd = sd(PPV),
            n= n()) %>% 
  ggplot(aes(x = min_len, y = TPR, color = tool)) +
  geom_point(alpha = 0.5) +
  geom_line() +
  facet_wrap(vars(window))  + 
  ylim(0, 1) 
```

### Minimum separation of integrations
```{r}
int_scores %>% 
  filter(experiment == "OTC-min_sep") %>% 
  group_by(condition, experiment, tool, window, min_sep) %>% 
  summarise(TPR = mean(TPR),
            TPR_sd = sd(TPR),
            PPV = mean(PPV),
            PPV_sd = sd(PPV),
            n= n()) %>% 
  ggplot(aes(x = min_sep, y = PPV, color = tool)) +
  geom_point(alpha = 0.5) +
  geom_line() +
  facet_wrap(vars(window))  + 
  ylim(0, 1) 
```

```{r}
int_scores %>% 
  filter(experiment == "OTC-min_sep") %>% 
  group_by(condition, experiment, tool, window, min_sep) %>% 
  summarise(TPR = mean(TPR),
            TPR_sd = sd(TPR),
            PPV = mean(PPV),
            PPV_sd = sd(PPV),
            n= n()) %>% 
  ggplot(aes(x = min_sep, y = TPR, color = tool)) +
  geom_point(alpha = 0.5) +
  geom_line() +
  facet_wrap(vars(window))  + 
  ylim(0, 1) 
```

```{r}
sessionInfo()
```

